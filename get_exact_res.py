...
sig_z = torch.eye(2, dtype=torch.complex128)
sig_z[1,1] = -1
#print("\sigma_z = \n", sig_z)

sig_x = torch.zeros((2,2), dtype=torch.complex128)
sig_x[1,0] = 1
sig_x[0,1] = 1
#print("\sigma_x = \n", sig_x)

sig_y = torch.zeros((2,2), dtype=torch.complex128)
sig_y[1,0] = 1j
sig_y[0,1] = -1j
...
def sigma_i(matrix, index, system_size):
  ...
  if (index%system_size > 0):
    sigma_i = torch.eye(2, dtype=torch.complex128)
    for i in torch.arange(1, index%system_size):
      sigma_i = torch.kron(torch.eye(2, dtype=torch.complex128), sigma_i)
...
def get_diag_hamiltonian_TFI(h_param, lattice_sites):
  n = lattice_sites
...
def get_mean_magn_op(lattice_sites, base_op):
  n = lattice_sites
  size = np.power(2,n)
  op = torch.zeros((size, size), dtype=torch.complex128)
  for i in range(n):
...
def calc_res(w, v, h_param, magn_op, corr_op_list, psi_init, lattice_sites, t_min=0, t_max=1):
  t_arr = torch.linspace(t_min,t_max,100)
  psi_t = [v @ torch.diag(torch.exp(-1j*w*t)) @ torch.conj(torch.transpose(v, 0, 1)) @ psi_init for t in t_arr]

  magn = [torch.conj(psi.unsqueeze(0)) @ magn_op @ psi.unsqueeze(1) for psi in psi_t]
  susc = torch.tensor([torch.autograd.grad(magn_val, h_param, retain_graph=True)[0] for magn_val in magn])
  ...
  return t_arr.numpy(), torch.real(magn).numpy(), susc.numpy(), torch.real(corr).numpy()
...
for h_param in h_param_list:
  w, v, h_param = get_diag_hamiltonian_TFI(h_param, lattice_sites)
  t_arr, magn, susc, corr = calc_res(w, v, h_param, magn_op, corr_op_list, psi_init, lattice_sites, t_max=3)
