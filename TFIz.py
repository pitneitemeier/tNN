...
if __name__=='__main__':
    ...
    test_cond = cond.ED_Test(magn_op, corr_list, h_list, lattice_sites, ED_magn, ED_susc, ED_corr, val_alpha, val_h_params, val_sampler, 
        name=f'TFI{init_polarization}', plot_folder=f'results/TFI{lattice_sites}{init_polarization}/', plot_fmt='.pdf', magn_op_name=r'\sigma^x', corr_op_name=r'\sigma^z')
    ...
    #model = models.TestModel(lattice_sites, num_h_params=1, learning_rate=3e-3, psi_init=psi_init_x_forward,
    #    act_fun=nn.GELU, kernel_size=3, num_conv_layers=3, num_conv_features=24,
    #    tNN_hidden=128, tNN_num_hidden=3, mult_size=1024, psi_hidden=80, psi_num_hidden=3, total_steps=2*steps, init_decay=1)
    ...
    trainer = pl.Trainer(fast_dev_run=False, gpus=[0,1], max_epochs=3, resume_from_checkpoint = f'TFI{lattice_sites}{init_polarization}_FFx_4.ckpt',
        auto_select_gpus=True, gradient_clip_val=.5, log_every_n_steps=10,
    ...
    #trainer.save_checkpoint(f'TFI{lattice_sites}{init_polarization}_FFx_4.ckpt')
