...
if __name__=='__main__':
    ...
    for l in range(lattice_sites):
        h1 += op.Sx(l) * (op.Sx((l+1) % lattice_sites)) + op.Sy(l) * (op.Sy((l+1) % lattice_sites))

    h2 = []
    for l in range(lattice_sites):
        h2 += op.Sz(l) * (op.Sz((l+1) % lattice_sites))
    ...
    h_param_range = [(0.15, 1.05), (0.15, 1.55)]
    ...
    name=f'XXZ {init_polarization}', plot_folder=f'results/XXZ{lattice_sites}{init_polarization}/', plot_fmt='.pdf')
    ...
    #model = models.ParametrizedFeedForward(lattice_sites, num_h_params=2, learning_rate=1e-3, psi_init=psi_init_x_forward,
    #    act_fun=nn.GELU, kernel_size=3, num_conv_layers=3, num_conv_features=24,
    #    tNN_hidden=128, tNN_num_hidden=3, mult_size=1024, psi_hidden=80, psi_num_hidden=3, step_size=8, gamma=0.1, init_decay=1)
    model = models.ParametrizedFeedForward.load_from_checkpoint('XXZ10x_FF_1.ckpt')
    ...
    neptune_logger = NeptuneLogger(
            ...
            name=f'XXZ{init_polarization}',
            close_after_fit=False
        )
    neptune_logger.log_hyperparams(h_param_dict)
    
    trainer = pl.Trainer(fast_dev_run=False, gpus=[0,1], max_epochs=24,
    ...
    #trainer.save_checkpoint(f'XXZ{lattice_sites}{init_polarization}_FF_1.ckpt')
